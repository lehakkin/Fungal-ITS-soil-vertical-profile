...
#write.csv2(all_top20_unique_df, file = "C:\\Users\\lehakkin\\PhD\\Fungal_ITS_soil_vertical_profile\\Heatmap_all_top20_unique_with_unclassified_df.csv", row.names = FALSE)
...
#save(ps_genus_nf_HETAMAP, file='ps_genus_nf_HETAMAP_all_top20_unclassified_included')
```
...
#write.csv2(all.sig.pvals, file = "C:\\Users\\lehakkin\\PhD\\Fungal_ITS_soil_vertical_profile\\HEATMAP_NO_CLUSTERS_Including_Unclassified_sig_wilcox_p_values.csv", row.names = FALSE)
...
#write.csv2(x, file = "V\\HEATMAP_sig_GENERA_NO_CLUSTERS_Including_Unclassified_mean_and_se.csv", row.names = FALSE)
...
#save(ps_FG_HEATMAP_no_clusters, file = 'ps_FG_for_HEATMAP_no_clusters_significant_Including_Unclassified')

```


### 13.1.3. START HERE FOR FIGURE: Load everything needed
...
all.sig.pvals <- read.csv2("HEATMAP_NO_CLUSTERS_Including_Unclassified_sig_wilcox_p_values.csv")

all.sig.genus <- unique(all.sig.pvals$genus)
length(all.sig.genus)

#load('ps_FG_for_HEATMAP_no_clusters_significant_Including_Unclassified')#ps_FG_HEATMAP_no_clusters

# first aggregate to genus
ps_RA_nf_genus <- aggregate_rare(ps_RA_nf, level = "genus", detection = 0/140, prevalence = 0/100)

#all_top20_unique <- read.csv2("C:\\Users\\lehakkin\\PhD\\Fungal_ITS_soil_vertical_profile\\Heatmap_all_top20_unique_with_unclassified_df.csv")

all_top20_unique <- all_top20_unique$all_top20_unique
...
#write.csv2(x, file = "C:\\Users\\lehakkin\\PhD\\Fungal_ITS_soil_vertical_profile\\HEATMAP_sig_GENERA_mean_and_se_FOREST_ONLY_all_10_most_abund_unclassified_included.csv", row.names = FALSE)
...
```{r}
setwd('V\\R_ITS_yoni\\Analyses_final\\RE_ANNOTATION_2024')
